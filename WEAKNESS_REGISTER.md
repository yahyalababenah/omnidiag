...
| **HM-5** | **`HeartDiseaseInput` required all 11 fields, but the shipped Pipeline's `ColumnTransformer` imputes every one of them (`IterativeImputer` for the 6 numeric columns, `SimpleImputer(strategy='most_frequent')` for the 5 categorical ones) — the schema was stricter than the model it validates for. Discovered via the same 12-row real-UCI `/batch` test used for FE-1/FE-2: `succeeded:7, failed:5` on a file with only genuinely missing cells (empty `ST_Slope` on 3 rows, empty `Cholesterol`/`FastingBS` on 2, typical for the Hungarian UCI site), not bad data.** | Exact per-row pydantic errors pulled directly, not assumed: 3× `ST_Slope: Input should be 'Up', 'Flat' or 'Down'`, 2× `Cholesterol`/`FastingBS: Input should be a valid integer`. Cross-checked against `evaluation_evidence/heart/shap_importance.json` before deciding which missing fields deserve a clinician-facing warning — the initial guess (`ChestPainType, Oldpeak, ExerciseAngina, ST_Slope` as "top 4") was wrong: the real ranking is `ChestPainType 0.969, Oldpeak 0.551, ExerciseAngina 0.524, Sex 0.407, Cholesterol 0.404, ..., RestingBP 0.220, **ST_Slope 0.135** (10th of 11), RestingECG 0.079`. ST_Slope — the feature that surfaced this whole bug — turned out to be one of the two *least* important, confirmed by both the JSON numbers and the SHAP beeswarm plot's row order. | — | **Fixed 2026-09-21:** `RestingBP, Cholesterol, FastingBS, MaxHR, Oldpeak, ST_Slope` are now `Optional[...] = Field(None, ...)` on `HeartDiseaseInput` (`Age, Sex, ChestPainType, RestingECG, ExerciseAngina` stay required). `backend/main.py::batch_predict`'s CSV coercion also had to change (`""` → `None`, not `""` → `""`) — an empty cell staying `""` still failed pydantic even after the field became Optional, since `Optional[X] = None` only defaults for an absent key or explicit `None`, not an empty string. Added `ModelLoader._completeness_warning()`: a response carries `data_completeness_warning` (both `/predict` and `/batch`, per-row) only when one of a **static, hand-picked** 5-feature list (`ChestPainType, Oldpeak, ExerciseAngina, Sex, Cholesterol` — chosen by the user from the real SHAP ranking, not auto-derived, and needing manual review if the model is ever retrained) is missing; the other Optional-but-low-rank fields (`ST_Slope`, `RestingBP`, `FastingBS`, `MaxHR`) succeed silently, exactly as before. Surfaced in the UI, not just the API: an amber banner in `ClinicalEmrMode.jsx` and a "Data Quality" column in `BatchUpload.jsx`'s results table (+ CSV export column) — a silent-in-the-logs-only warning was explicitly rejected as insufficient. Verified end-to-end on the real 12-row file: `succeeded:12, failed:0` (was 7/12); rows with a missing high-impact field (`Cholesterol`) carry the warning, rows missing only low-impact fields (`ST_Slope`) don't. 5 new regression tests (low-impact silent, high-impact warned, complete row unwarned). 298/298 tests pass (293 + 5). | S1 | **FIXED** |
| **HM-6** | **The external-validation ROC-AUC quoted everywhere for the heart module (72.17%, Tehran/Z-Alizadeh Sani cohort) was measured before the Optuna hyperparameter tuning that produced the shipped `heart_full_tuned.pkl` — it had never been re-run against a tuned checkpoint.** Surfaced while reconciling documentation sources during the 2026-09-21 doc-refresh pass: `evaluation_evidence/heart/final_metrics.json`'s `operating_point.threshold` (0.3104) and `evidence_files` (`models/heart_full.pkl`, not `_tuned`) don't match `configs/heart_disease.yaml`'s deployed threshold (0.3695, from `hpo_threshold_summary.json` / `heart_full_tuned.pkl`). | Confirmed by reading both evidence files directly: `final_metrics.json.operating_point.threshold = 0.3104` vs `configs/heart_disease.yaml: inference_threshold: 0.3695` vs `hpo_threshold_summary.json.threshold.shipped = 0.3695` (matches config). `final_metrics.json.headline.external_tehran.roc_auc = 72.17` is the pre-tuning number. **Important precision, caught before it was mislabeled in this very fix:** Tehran's data is missing 4 of the full model's 11 features (`MaxHR, Oldpeak, ST_Slope, ExerciseAngina` — `UNAVAILABLE` per `external_validation.json`'s own mapping table), so **no** version of the 11-feature full model (tuned or not) can ever be scored on Tehran directly — only the 7-feature reduced model family can. A first re-measurement was submitted labeled `"model": "heart_full_tuned.pkl"`; it was actually `heart_reduced_tuned.pkl` (its own threshold, 0.4237, is `hpo_threshold_summary.json`'s documented `shipped_reduced` value, not `shipped`) — caught and corrected before being written into any doc. | — | **Re-measured and fixed 2026-09-21:** `evaluation_evidence/heart/external_validation_tuned.json` — `heart_reduced_tuned.pkl` (Optuna-tuned, 7 shared features, threshold 0.4237) scores **76.34% ROC-AUC** on Tehran, **+4.17 points over the untuned reduced model's 72.17%** (sensitivity 55.09%, specificity 81.61%, FN 97, FP 16, n 303) — tuning improved external transfer, it did not just change the internal LOSO number. Every doc updated to cite this as *"76.34% (reduced model, 7 features, on Tehran)"*, explicitly not a measurement of the deployed 11-feature `heart_full_tuned.pkl`'s general performance — the two are different model families and must not be conflated. The 11-feature model's own external-validation gap remains structurally unmeasurable on this cohort (see the note above) and is not claimed fixed. | S2 | **RESOLVED — remeasured on the correctly-identified tuned reduced model, mislabeling caught before publication** |

**Part 7 counts (6 rows):** OPEN 2 · FIXED 3 · RESOLVED 1. Not included in the Counts table above.

---

# PART 8 — FRONTEND: BATCH UPLOAD AUTH ROUTING (added 2026-09-21)

> **Context.** Reported as "Batch Prediction fails with 'Failed to execute json on Response:
> Unexpected end of JSON input'". Diagnosed initially by reproducing against the real backend
> (`/api/v4/heart_disease/batch` with both files at hand) -- the endpoint itself handled every
> file correctly (HM-2/HM-3 above). The real cause was found by direct `curl` against the live
> endpoint: a clean `401 Not authenticated`, not a data or model problem, disproving the earlier
> working hypothesis before writing any fix. Disease-agnostic (affects heart and diabetes batch
> uploads identically) so it does not fit Part 7; own ID prefix.
>
> **ID prefix:** `FE-` (frontend). Independent of `W-`, `H-`, `D-`, `P-`, `HM-`.

| ID | What | Measured / evidence | Why not fixed | What would fix it | Sev | Status |
|---|---|---|---|---|---|---|
| **FE-1** | **`BatchUpload.jsx` sent its request to a relative URL (`const BASE = '/api/v4'`), the only *live, rendered* component in the app that does — every other hand-rolled `fetch()` (`ClinicalNotesInput.jsx`, `ReviewQueuePanel.jsx`, `AdminDashboard.jsx`, `AuthContext.jsx`) targets the absolute `API_BASE` from `api.js` (`https://yahyoha-omnidiag.hf.space` by default). A relative path resolves against the frontend's own origin, not the backend's -- in any cross-origin deployment (confirmed the real one, since every other component assumes it) the request never reaches the authenticated backend at all. The `Authorization: Bearer ${token}` header itself was already attached correctly in the code; it just never reached anything that checks it. `PatientTimeline.jsx` (imported and rendered by `ClinicalEmrMode.jsx` -- live, not to be confused with the dead `PatientRiskTimeline.jsx` from P-15) has the identical `const BASE = '/api/v4'` pattern and was not fixed here -- flagged, not in scope of the reported bug.** | Verified end-to-end against a real local backend instance (not assumed): with the old relative-URL code, requests to `/api/v4/*` from a different origin cannot reach it -- structurally, not intermittently. After the fix, ran the *exact* fixed fetch logic (same URL construction, same headers) from Node against a live backend with a seeded doctor account: authenticated request -> `200`, `total: 12, succeeded: 7, failed: 5`; **confirmed server-side in `audit_logs`**, not just client-side: `('/api/v4/heart_disease/batch', 'POST', 200, '<doctor's real user_id>')` for the authenticated call vs `(..., 401, None)` for a no-token control call -- the header is attached and accepted. | — | **Fixed 2026-09-21:** `BatchUpload.jsx` now imports `API_BASE` from `../api` and builds `` `${API_BASE}/api/v4` `` instead of the bare relative path, matching every other component. `npm run build` succeeds. | S1 | **FIXED** (`PatientTimeline.jsx`'s identical pattern is a separate, unfixed occurrence -- see note above) |
| **FE-2** | **The generic `res.json()` parse failure ("Unexpected end of JSON input") was the *only* error message the UI could ever show, for every possible backend failure -- 401, 422, 500, a proxy timeout, a non-JSON error page -- because `BatchUpload.jsx` called `res.json()` directly and let it throw on any non-JSON or empty body, discarding the real HTTP status and any server-supplied message before the catch block ever saw them.** | Direct comparison: the real backend's 401 body is `{"detail": "Not authenticated — provide a Bearer token, cookie, or X-API-Key"}` -- valid, informative JSON. The UI nonetheless showed a JavaScript-internals string that names no status code and no cause, indistinguishable from a timeout, a corrupt file, or a server crash. Confirmed by reproducing the fixed code's error path against a real no-token 401 response. | — | **Fixed 2026-09-21, same commit as FE-1:** reads the response as text first, parses it as JSON only if non-empty, and on failure raises `` `${serverMessage} (HTTP ${status})` `` when the body carries a `detail`/`error` field, or a clear `` `Request failed — HTTP ${status} ${statusText}` `` fallback when it doesn't -- never the opaque parse-failure message again. Verified: the same no-token request that used to surface "Unexpected end of JSON input" now surfaces `"Not authenticated — provide a Bearer token, cookie, or X-API-Key (HTTP 401)"`. | S2 | **FIXED** |

**Part 8 counts (2 rows):** FIXED 2. Not included in the Counts table above.

---

# PART 9 — MODEL-FAMILY REGISTRY REFACTOR (added 2026-09-21)

> **Context.** Model loading used to be selected by whether a config had an `ensemble:` key,
> and `model.type` was decorative. Adding a disease also required editing the hard-coded
> `DISEASE_SCHEMA_REGISTRY` in `backend/schemas.py`. The work is on branch
> `feat/model-family-registry` only; nothing is merged into `deploy/v2-platform`.
>
> **ID prefix:** `AR-` (architecture). Independent of the other prefixes.

| ID | What | Measured / evidence | Why not fixed | What would fix it | Sev | Status |
|---|---|---|---|---|---|---|
| **AR-1** | **Config-driven model-family registry.** `model.family` in YAML is now the dispatch key: `@register_backend` / `get_backend()` in `backend/model_backends/`, and an unknown or missing family fails startup with a message that lists the registered families. Heart and diabetes run through `SklearnPipelineBackend` and `StackingEnsembleBackend`, which *subclass* `ModelLoader` / `EnsembleModelLoader` without changing them. `/batch` and `supports_counterfactuals` read backend capabilities instead of `hasattr` or `ensemble:` checks. An optional YAML `schema: {module, class}` registers a disease's input schema. Label handling, prevalence correction, thresholds, risk bands and completeness warnings were not moved. | **Tests:** 298 → 322 passed (24 new). **Golden master** (`scratch/golden_master.py`, real router and app; /predict, /explain, /batch, /schema, /diseases, /api/v3/predict for heart P-001..3 and diabetes A/B/C/D, plus 3 edge cases each): `golden_before.json` vs `golden_after.json` **IDENTICAL**, 1462 values, tol 1e-9. Counterfactuals are compared by shape and count only, because their content differs between two runs of the *unchanged* code. An interim run did show D-003 (Case C) at 1 scenario instead of 3. Traced to that pre-existing nondeterminism: with `PYTHONHASHSEED` fixed, old code (git worktree at `7d344b5`) and new code gave byte-identical full CF content (seeds 0/1/2), and the old code alone gave two different outputs at seed 0. The strict seeded golden diff (`--strict`, CF content included, 1683 values) differs only in the checkout path inside one traceback string. **Third family proof:** `demo_logreg`, a Pipeline(imputer→scaler→LogisticRegression) trained in a test fixture, was registered by YAML in a tmp dir and served through the real app. /schema, /predict and /explain (generic SHAP, additive to 1e-6) work, /counterfactuals returns 501. Cost: new family = 1 file, 54 lines (29 code); new disease in that family = 13 YAML lines + 7-line schema, 0 `backend/` edits (git-checked in the test). `npm run build` OK; live uvicorn run hit every endpoint for both diseases with 0 ERROR lines in the log. **Found while verifying, not fixed (out of scope):** (a) diabetes `/counterfactuals` content is not reproducible across processes (set iteration order under hash randomisation, plus occasional inference jitter); (b) heart `/counterfactuals` returns 500 when an Optional field is missing (`int(None)` in `ModelLoader.generate_counterfactuals`, same before and after). | The "config only" claim is proven for `sklearn_generic` only. `sklearn_pipeline` still hard-codes heart's `_HIGH_IMPACT_FEATURES`, counterfactual ranges and `prep`/`clf` step names, and `stacking_ensemble` hard-codes diabetes's engineered-feature names. These were left in place on purpose (move only if required). | Move those lists to YAML before registering a second disease in either built-in family. | — | **DONE on feature branch — not merged** |

---

# PART 10 — WHAT-IF CLINICAL VALIDITY + UI SMOKE-TEST FIXES (added 2026-09-21)

> **Context.** A live production smoke test found clinically invalid What-If recommendations
> and five related UI issues. Fixed on branch `fix/whatif-clinical-validity` (from
> `deploy/v2-platform` at `c0c3e85`). Not merged. Models, thresholds, prevalence correction
> and SHAP are unchanged: `/predict`, `/explain`, `/batch`, `/schema`, `/diseases` and
> `/api/v3/predict` are **identical** before and after (`scratch/golden_pre_whatif.json` vs
> `golden_post_whatif.json`, 1307 values, tol 1e-9, `PYTHONHASHSEED=0`). Tests: 322 → 348.
>
> **ID prefix:** `WI-` (What-If / UI). Independent of the other prefixes.

| ID | What | Measured / evidence | Why not fixed | What would fix it | Sev | Status |
|---|---|---|---|---|---|---|
| **WI-1** | **What-If recommended clinically impossible or harmful changes.** Diabetes: BMI *gain* (seen in the baseline as D-003 BMI 33 → 33.67), GenHlth 3 → 4 (D-004), more poor-mental-health days, and "undoing" HighBP, HighChol, Smoker (BRFSS "ever told" / "ever smoked 100 cigarettes", which is history, not a lever). Heart: MaxHR, Oldpeak and ExerciseAngina were levers. | **Fixed.** `DIABETES_POLICY` (`backend/counterfactual_generator.py`): only BMI (decrease, floor 18.5), PhysActivity, Fruits, Veggies (→1) and HvyAlcoholConsump (→0) may change; 16 features are immutable. `HEART_POLICY` (`ModelLoader.generate_counterfactuals`): RestingBP (decrease, floor 110), Cholesterol (decrease, floor 150), FastingBS (→0). Both stay hard-coded where they already were. The policy is enforced at sampling (BMI one-sided) **and** by `policy_violations()` on every scenario before return. Changes are detected across *all* features, so a hidden change to an immutable feature is rejected rather than silently scored. `tests/test_whatif_policy.py` checks every demo patient and extra positives against an independent copy of the policy, injects violating candidates into both generators, and re-predicts each scenario from its reported changes only. | — | — | S1 | **FIXED** |
| **WI-2** | **Heart What-If for P-002 (96.5%) rendered an empty card reading "reduces relative risk by up to 0%".** | **Diagnosed: no error.** 774 `predict` calls, 0 exceptions; the lowest probability the random search reached was 0.520, above the 0.3695 threshold, so the result was an honest empty list rendered badly. **Fixed:** when nothing crosses, the response carries `crosses_threshold: false` and `best_achievable` (every allowed lever improved: P-002 0.965 → 0.871; D-003 0.637 → 0.423). The card shows it with the referral message. If the patient has no lever, a distinct message says so. The subtitle "…not a predicted treatment effect" was added, and wording no longer assumes 3 scenarios. | — | — | S1 | **FIXED** (UI render UNVERIFIED in a browser; build only) |
| **WI-3** | **Diabetes counterfactuals were non-deterministic across processes** (set iteration under `PYTHONHASHSEED`). | **Fixed** with `sorted()` iteration. `TestDeterminism` runs two processes (hash seeds 1 and 4242): all demo scenarios and `best_achievable` are identical. Residual: D-003's *baseline* probability differed by 4.4e-16 between two processes in one manual run (ensemble float jitter); no scenario changed. Also, one scenario per changed-feature set: D-004 had returned two identical BMI → 23.9 scenarios. | Float jitter is inside the base models, out of scope. | — | S2 | **FIXED** |
| **WI-4** | **Heart `/counterfactuals` returned HTTP 500 when an Optional field was missing** (`int(None)`). | **Fixed:** a `None` lever is skipped, never guessed. Golden edge case `edge_missing_high_impact`: 500 → 200. Regression tests cover all-missing and each lever missing individually. | — | — | S2 | **FIXED** |
| **WI-5** | **"Engineered Feature: Diabetes_Clinical_Risk" panel rendered on the heart page.** | **Fixed:** the panel renders only when the disease's own `/explain` feature list contains `Diabetes_Clinical_Risk`. There is no disease-name check. Exposing engineered features via `/diseases` would need an API change and was not done. | — | — | S3 | **FIXED** |
