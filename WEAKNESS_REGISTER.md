...
| **HM-6** | **The external-validation ROC-AUC quoted everywhere for the heart module (72.17%, Tehran/Z-Alizadeh Sani cohort) was measured before the Optuna hyperparameter tuning that produced the shipped `heart_full_tuned.pkl` — it has never been re-run against the actual deployed model.** Surfaced while reconciling documentation sources during the 2026-09-21 doc-refresh pass: `evaluation_evidence/heart/final_metrics.json`'s `operating_point.threshold` (0.3104) and `evidence_files` (`models/heart_full.pkl`, not `_tuned`) don't match `configs/heart_disease.yaml`'s deployed threshold (0.3695, from `hpo_threshold_summary.json` / `heart_full_tuned.pkl`) — two different model checkpoints, evaluated separately, with only the untuned one ever run against Tehran. | Confirmed by reading both evidence files directly: `final_metrics.json.operating_point.threshold = 0.3104` vs `configs/heart_disease.yaml: inference_threshold: 0.3695` vs `hpo_threshold_summary.json.threshold.shipped = 0.3695` (matches config). `final_metrics.json.headline.external_tehran.roc_auc = 72.17` is the only Tehran number in the repo, and its own `evidence_files` list names the pre-tuning models. | Re-running Tehran validation means loading `heart_full_tuned.pkl`, applying the same 7-shared-feature mapping `external_validation.json` already documents, and scoring — a measurement task, not a code fix, and out of scope for a documentation-accuracy pass. | Re-run the existing Tehran external-validation script against `heart_full_tuned.pkl` specifically and replace the 72.17% figure (or confirm it, if unchanged) with a number attributable to the actually-shipped model. | S2 | **OPEN — external validation not yet re-run on the tuned model** |

**Part 7 counts (6 rows):** OPEN 3 · FIXED 3. Not included in the Counts table above.
