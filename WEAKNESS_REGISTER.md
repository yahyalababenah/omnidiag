...
| **P-15** | **`frontend/src/components/ReviewQueuePanel.jsx` is not rendered anywhere.** No component imports it; the admin dashboard has its own review table. | `grep -rn ReviewQueuePanel frontend/src` finds only the file itself. Its NULL-row note was corrected on 2026-09-19 (a legacy 50%-centred score is still comparable for an argmax module like heart, not for diabetes), but that text is currently invisible. | Deleting or wiring a component is a product decision. | Wire it in, or delete it. | S4 | **OPEN** |
...
**Part 6 counts (15 rows):** OPEN 10 (of which 1 decision-needed, 1 manual check, 1 scheduled) · PARTIALLY FIXED 1 · ACCEPTED 1 · BY DESIGN 1 · FIXED 2. Not included in the Counts table above.
