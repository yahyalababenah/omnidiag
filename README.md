...
|---|---|---|---|
| `POST` | `/api/v4/{disease}/predict` | Anonymous OK¹ | `{prediction, confidence, diagnosis, uncertainty_band, shap_chart_data, queued_for_review, cached}` |
| `POST` | `/api/v4/{disease}/explain` | Anonymous OK¹ | `{shap_values[], text_explanation, base_value}` |
| `POST` | `/api/v4/{disease}/counterfactuals` | Anonymous OK¹ | `{counterfactuals[{changes, feasibility}]}` |
| `POST` | `/api/v4/{disease}/batch` | `doctor`/`nurse`/`super_admin` | `{results[], summary{total, ok, errors}}` — CSV upload, max 500 rows |
| `POST` | `/api/v4/generate-report` | Anonymous OK¹ | `{report, source, llm_model?, latency_ms?, fallback_reason?}` |
